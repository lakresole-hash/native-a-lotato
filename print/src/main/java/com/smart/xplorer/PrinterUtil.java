...
import static com.smart.xplorer.SmartPrinter.CENTER;
import static com.smart.xplorer.SmartPrinter.FULL_WIDTH;
import static com.smart.xplorer.SmartPrinter.ORIGINAL_WIDTH;
import static com.smart.xplorer.SmartPrinter.RIGHT;
...
class PrinterUtil {
    ...
    boolean printTextTitle(String text) {
        try {
            byte[] format = {0x1b, 0x21, 0x10};
            byte[] format1 = {0x1b, 0x21, 0x08};
            String s = StrUtil.encodeNonAscii(text);
            btOutputStream.write(format);
//            btOutputStream.write(format1);
            btOutputStream.write(s.getBytes(), 0, s.getBytes().length);
            return true;
        } catch (IOException e) {
            e.printStackTrace();
            return false;
...
//            btOutputStream.write(format);
    ...
    public static byte[] qrCodeCommand(String data) throws Exception {
    ...
    boolean printBarCode(String text) {

        try {

            byte[] alignCenter  = {0x1D,0x61,1};
            byte[] backLeft  = {0x1D,0x61,0};
            byte[] height = {0x1D,0x68,(byte) 150};
            byte[] width  = {0x1D,0x77,4};
            byte[] textBelow   = {0x1D,0x48,2}; 
            byte[] barcode = {0x1D, 0x6B, 73, (byte) text.length()};

            btOutputStream.write(alignCenter);
            btOutputStream.write(height);
            btOutputStream.write(width);
            btOutputStream.write(textBelow);

            btOutputStream.write(barcode);
            btOutputStream.write(text.getBytes());

            btOutputStream.write("\n\n".getBytes());
            btOutputStream.write(backLeft);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return true;
    ...
    boolean qrCode(String text) {
        try{
            btOutputStream.write(qrCodeCommand(text));
    ...
    boolean printTextTall(String text) {
        try {
            byte[] format = {0x1B, 0x21, 0x10};
            String s = StrUtil.encodeNonAscii(text);
            btOutputStream.write(format);
    ...
    boolean printTextBold(String text) {
        try {
            byte[] format = {0x1B, 0x21, 0x08};
            String s = StrUtil.encodeNonAscii(text);
            btOutputStream.write(format);
            btOutputStream.write(s.getBytes(), 0, s.getBytes().length);
            return true;
    ...
    boolean printTextWide(String text) {
        try {
            byte[] format = {0x1B, 0x21, 0x20};
            String s = StrUtil.encodeNonAscii(text);
            btOutputStream.write(format);
            btOutputStream.write(s.getBytes(), 0, s.getBytes().length);
            return true;
    ...
    int addNewLine(int count, ProgressDialog progressDialog) {
        int success = 0;
        for (int i = 0; i < count; i++) {
            if (addNewLine()) success++;
        }
    ...
    private Bitmap scaledBitmap(Bitmap bitmap, int width) {
        try {
            int desiredWidth = width == 0 || bitmap.getWidth() <= PRINTER_WIDTH
                    ? bitmap.getWidth() : PRINTER_WIDTH;
